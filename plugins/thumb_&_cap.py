...
async def handle_file_rename(client: Client, message: Message):
    """Handle file renaming with points system"""
    try:
        ...
        if not premium_status.get('is_premium', False):
            points_per_rename = 2  # Default cost
            current_points = await codeflixbots.get_points(user_id)
            ...
            await codeflixbots.deduct_points(user_id, points_per_rename, "file_rename")
        
        # Get rename template
        template = await codeflixbots.get_format_template(user_id)
        if not template:
            if not premium_status.get('is_premium', False):
                await codeflixbots.add_points(user_id, points_per_rename, "refund", "No template")
            return await send_response(
                client,
                message.chat.id,
                f"{EMOJI['error']} **No rename template set!**\n"
                "Use /autorename to set your template first\n\n"
                f"{f'{points_per_rename} points refunded' if not premium_status.get('is_premium', False) else ''}",
                delete_after=30
            )
        ...
        new_name = new_name.replace("[size]", f"{round(file_size / (1024 * 1024), 2)}MB")
        new_name = new_name.replace("[duration]", f"{duration // 60}m{duration % 60}s")
        new_name = new_name.replace("[date]", datetime.now().strftime("%Y-%m-%d"))
        new_name = new_name.replace("[time]", datetime.now().strftime("%H:%M"))
        new_name += os.path.splitext(original_name)[1]  # Add extension
        
        # Track rename in database
        await codeflixbots.track_file_rename(user_id)
        
        # Send success message
        success_msg = await send_response(
            client,
            message.chat.id,
            f"{EMOJI['success']} **File renamed successfully!**\n\n"
            f"📝 **Original:** {original_name}\n"
            f"🆕 **New name:** {new_name}\n\n"
            f"{f'⏳ **Points deducted:** {points_per_rename} {EMOJI['points']}\n' if not premium_status.get('is_premium', False) else ''}"
            f"💰 **Remaining balance:** {await codeflixbots.get_points(user_id)} {EMOJI['points']}",
            delete_after=30
        )
